FROM docker:20.10.13-dind-alpine3.15

# Install glibc
RUN apk del libc6-compat && \
    apk --no-cache --update add curl==7.80.0-r0 binutils==2.37-r3 && \
...
RUN apk --no-cache --update add libxml2-dev==2.9.13-r0 libxslt-dev==1.1.35-r0 libffi-dev==3.4.2-r1 build-base==0.5-r2 && \
